...
export default function PropertyDetailScreen() {
    ...
    return (
        <View className="flex-1 bg-white">
            <ScrollView showsVerticalScrollIndicator={false} bounces={false} contentContainerStyle={{ paddingBottom: Math.max(insets.bottom + 20, 40) }}>
                {/* Image Carousel */}
                <View>
                    ...
                    {isAdmin && (
                        <View className="gap-3">
                            <View className="flex-row gap-3">
                                <TouchableOpacity
                                    onPress={() => router.push(`/(root)/property/edit?id=${property.id}`)}
                                    className="flex-1 flex-row items-center justify-center gap-2 bg-blue-50 py-4 rounded-2xl border border-blue-200"
                                >
                                    <Ionicons name="create-outline" size={18} color="#2563EB" />
                                    <Text className="text-blue-600 font-semibold">Edit Property</Text>
                                </TouchableOpacity>
